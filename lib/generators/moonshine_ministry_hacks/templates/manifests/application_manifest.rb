...
class ApplicationManifest < Moonshine::Manifest::Rails
  ...
  def certs
    ...
    for domain in %w(powertochange.org campusforchrist.org)
      for type in %w(cert key)
        file_name = "/etc/apache2/certs/#{domain}.#{type}"
        file filename,
          :ensure => :present,
          :source => "/tmp/moonshine_config_files/#{domain}.#{type}",
          :owner => "root" if File.exists?(filename)
      end
    end
  end
